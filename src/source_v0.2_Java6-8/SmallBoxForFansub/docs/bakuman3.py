...
ScriptPlugin.pythonRegister("Slow Fall (array mode)", "bakuman", "1.0", "no description", "Chien-Rouge", "", "")
...
def bakuman():
    ...
    for line in lines:
        ...
        while counter<len(osyl):
            ...
            sextra = str(osyl[counter][4]+1000)                       

            # The first syllable may contains overrides
            if counter==0:
                newsentence = "{\\an5\\pos("+posx+","+posy+")\\org(-10000,"+posy+")\\1c&H0000CC&\\3c&HCCCCFF&\\bord5\\blur5\\t("+sstart+","+sextra+",\\frz-0.25\\1c&HCC0000&\\3c&HFFFFFF&\\alpha&HFF&)}"+syl
            # All others syllables
            else:
                newsentence = newsentence + "{\\r\\1c&H0000CC&\\3c&HCCCCFF&\\bord5\\blur5\\t("+sstart+","+sextra+",\\frz-0.25\\1c&HCC0000&\\3c&HFFFFFF&\\alpha&HFF&)}"+syl
        ...
        while counter<len(osyl):
            ...
            # The first syllable may contains overrides
            if counter==0:
                newsentence = "{\\an5\\pos("+posx+","+posy+")\\1c&H0000CC&\\3c&HCCCCFF&\\bord5\\blur5\\alpha&HFF&\\3a&H00&\\t("+sstart+","+send+",\\fsc300\\3a&HFF&)\\t("+send+","+send+",\\fsc100)}"+syl
            # All others syllables
            else:
                newsentence = newsentence + "{\\r\\1c&H0000CC&\\3c&HCCCCFF&\\bord5\\blur5\\alpha&HFF&\\t("+sstart+","+sstart+",\\3a&H00&)\\t("+sstart+","+send+",\\fsc300\\3a&HFF&)\\t("+send+","+send+",\\fsc100)}"+syl
